...
export const AppLocalizeBehavior = {
  ...
  __localizationCache: {
    ...
    /* Unique localized strings. Invalidated when the language, formats or resources change. */
    ajax: null
  ...
  loadResources: function (path, language, merge) {
    ...
    this.__checkLocalizationCache(proto); // If the global ajax object has not been initialized, initialize and cache it.
    ...
    if (!request) {
      ...
      request.completes.then(onRequestResponse.bind(this), this.__onRequestError.bind(this)); // Cache the instance so that it can be reused if the same path is loaded.

      proto.__localizationCache.requests[path] = request;
    } else {
      request.completes.then(onRequestResponse.bind(this), this.__onRequestError.bind(this));
    }
  },
  ...
  __computeLocalize: function (language, resources, formats) {
    ...
    return function () {
      var key = arguments[0];
      if (!key || !resources || !language || !resources[language]) return; // Cache the key/value pairs for the same language, so that we don't
  ...
  __onRequestResponse: function (event, language, merge) {
    ...
    if (merge) {
      if (language) {
        ...
        propertyUpdates['resources.' + language] = Object.assign(propertyUpdates.resources[language] || {}, newResources);
      } else {
