...
`;
  ...
  _citypoints() {
    ...
    for (var i = 0; i < this.data.length; i++) if (C.eqp(data, this.data[i].p)) a.push(this.data[i].city);
  ...
  _extracttechnologies() {
    ...
    for (var i = 0; i < this.data.length; i++) if (C.eqp(figure, this.data[i].p) && C.eqp(city, this.data[i].city)) return this.data[i].tech;
  ...
  _clearcity() {
    ...
    this.$.city.draw(null); //    C.displayelem(this.$.button, false);
  }
