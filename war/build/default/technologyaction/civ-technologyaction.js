...
`;
  ...
  spendButton() {
    const pa = {}; // city not important if tech without city
  ...
  clickTech(d) {
    ...
    const tech = CC.getTechnologyAction(d.tech); // with city

    C.setcurrentcommand(tech.name);
    if (this._isCity()) C.itemizecommand(tech.name);else this._drawResources();
  }
  ...
  _activateSpendButton(display) {
    if (display) {
      const da = CC.getActionTechnology(C.getcurrentcommand());
      if (da.button == null) this.spendbutton = C.localize('spendresourcebutton');else this.spendbutton = C.localize(da.button);
    }
